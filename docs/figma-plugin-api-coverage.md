...
| Dev Mode codegen | optional offline `manifest.codegen.json` projects Bridge source intent, designer annotations, native CSS and component contracts | Read-only; repository paths and accepted baselines stay in `figma-bridge.json` and resolve through `link context` |
