...
describe('reuse handle parsing', () => {
  it('parseReuseLine round-trips with reuseHandleLine', () => {
    const line = reuseHandleLine({ key: 'PLACEHOLDERCOMPONENTKEY', id: '10:5' });
    assert.deepEqual(parseReuseLine(line), { key: 'PLACEHOLDERCOMPONENTKEY', id: '10:5' });
  });
  ...
  it('parseComponentSpecs picks up reuse handle', () => {
    const md = [
      '### Button',
      'Page: P · 2 variants',
      'Reuse: import existing — key `PLACEHOLDERCOMPONENTKEY` · node `10:5`',
      '',
      '| Property | Values |',
      '|---|---|',
      '| Size | S, M |',
    ...
    assert.deepEqual(specs[0].reuse, { key: 'PLACEHOLDERCOMPONENTKEY', id: '10:5' });
  });
});
