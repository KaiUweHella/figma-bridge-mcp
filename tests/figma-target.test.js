...
test('Figma Target Context resolves precedence once and stays immutable', () => {
  ...
  assert.deepEqual(explicit, { kind: 'plugin-file', fileKey: 'EXPLICITFILEKEY', source: 'explicit' });
  ...
  assert.deepEqual(inferred, { kind: 'plugin-file', fileKey: 'ARGUMENTFILEKEY', source: 'figma-url' });
  assert.equal(targetFileKey(inferred), 'ARGUMENTFILEKEY');
});
...
test('implicit single-window targeting is explicit in the Context', () => {
  ...
  assert.equal(targetFileKey('https://www.figma.com/board/BOARDFILEKEY/FILE_NAME'), 'BOARDFILEKEY');
});
